...
**基线**：Sandbox 基础隔离、Responses、MCP stdio、仓库上下文、checkpoint/恢复和审计已合入（2026-09-12）
...
1. 任务状态机与强制验证闭环。
2. 持久任务恢复 CLI/API 和跨平台 sandbox 纵深补强。
3. Skills、Anthropic adapter、网络 Capability 和多会话任务。
...
| 缺失 | 任务级完成判定、Anthropic adapter、精细网络策略、完整平台纵深隔离和 Skills | 全局 |
...
| 沙箱 | A3 | O2/O8 | Rust Helper 按平台探测并启用基础 namespace/AppContainer、资源限制和 Fail Closed | 完成 `/proc`、设备、网络、注册表和崩溃恢复等精细策略 |
...
| 审计 | A3/A5 | O2/O3/O4 | SQLite append-only `audit_events` 保存受限运行摘要 | 增加可检索查询、保留策略和脱敏验证 |
...
1. 已增加流式模型事件和 CLI 状态摘要。
2. 已为可重试模型错误加入次数和预算上限的退避策略。
3. 已持久化 run、tool batch、sandbox 和模型尝试摘要。
4. 已引入 token/字符预算、上下文摘要、artifact 和 checkpoint。

### P2：恢复与扩展（部分完成）

1. checkpoint、过期 run 恢复和幂等工具结果已完成；任务级恢复接口仍待补齐。
2. OpenAI Responses adapter 已完成；Anthropic adapter 待实现。
3. 本地 MCP stdio 已完成；Skills 待实现。
4. 基础平台 sandbox 已完成；精细网络策略、多会话、并发任务和管理员策略待实现。
