...
| 其他 provider adapter | **未实现** | Anthropic、OpenAI Responses 等 | 当前无实现 |
...
Adapter 把内部 `Message` 映射为 provider message，把模型工具定义映射为函数工具；响应中的文本、tool calls、finish reason 和 usage 再归一化为 `ModelResponse`。
...
当前没有通用重试和退避；失败会结束本次 Agent run，由上层决定是否重新发起新的 run。

## 7. 持久化 / 审计

模型请求摘要通过 `onApprovalRequired` 回调提供。当前不持久化完整 prompt、API key、认证头或原始响应；成功的 assistant 消息可随 Session 消息持久化。
...
**已实现**OpenAI-compatible Chat Completions、配置校验和逐次审批；**部分实现**usage/context 字段；**未实现**流式请求、重试退避和其他 provider；这些属于后续计划。
