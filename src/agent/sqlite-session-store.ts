  ...
  private migrate(): void {
    ...
    this.transaction(() => {
      this.database.exec(`
        ...
        CREATE TABLE runs (id TEXT PRIMARY KEY, session_id TEXT NOT NULL REFERENCES sessions(id), status TEXT NOT NULL CHECK(status IN ('running', 'completed', 'failed', 'interrupted')), input TEXT NOT NULL, final_text TEXT, error TEXT, started_at TEXT NOT NULL, finished_at TEXT, result_json TEXT);
...
interface RunRow { id: string; session_id: string; status: PersistedRunStatus; input: string; final_text: string | null; error: string | null; started_at: string; finished_at: string | null; result_json: string | null; }
